...
## Write table
t <- data.frame("date" = "201912", "sample_id" = sample)
t1 <- t(as.data.frame(total_protein_expression))
t2 <- cbind.data.frame(t,t1)
rownames(t2) <- NULL

t2_out = paste('~/GoogleDrive/gastric_cancer_samples/Tables/total_protein_expression.20200312/201912_total_protein_expression', sample, date, 'txt', sep='.')
